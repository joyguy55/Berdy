  ...
  render() {
    ...
    return (
      <div className="form-wrapper">
        <TextField className="search" hintText="Find a Movie" fullWidth={true} onChange={(e, newValue) => {
          this.props.searchApi(newValue)
        }}/>
        <div className="movies">
          {movie_arr.map((obj, ind) => {
            return (<img className="movie" key={obj.id} alt='' src={`https://image.tmdb.org/t/p/w154//${obj.poster_path}`} onClick={(e) => {
              this.handleMovieClick(ind)
            }}/>)
          })
}
        </div>
        {this.state.movieSelection !== null
          ? <Link to="/add2">
              <RaisedButton className="next" fullWidth={true} label="Next"/>
            </Link>
          : ''
}
      </div>
